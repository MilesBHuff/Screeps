...
creepLimitsGlobal.workers+=null!=room.memory.workerLimit?room.memory.workerLimit:1,creepLimitsGlobal.fighters+=null!=room.memory.fighterLimit?room.memory.fighterLimit:0,creepLimitsGlobal.claimers+=null!=room.memory.claimerLimit?room.memory.claimerLimit:0}})(),!spawn.spawning&&(spawn.room.energyAvailable>=spawn.room.energyCapacityAvailable||creepsLocal.workers.length<=0||creepsLocal.fighters.length<=0&&spawn.room.find(FIND_HOSTILE_CREEPS).length>0)&&(function trySpawnCreep(){for(let i=0;i<spawn.room.memory.workerLimit||i<spawn.room.memory.fighterLimit||i<spawn.room.memory.claimerLimit;i++){if(creepsLocal.workers.length<=i&&creepsLocal.workers.length<spawn.room.memory.workerLimit&&creepsGlobal.workers.length<creepLimitsGlobal.workers)roleSpawn.spawnCreep(spawn,LIB_MISC.ROLES.WORKER)
else if(creepsLocal.fighters.length<=i&&creepsLocal.fighters.length<spawn.room.memory.fighterLimit&&creepsGlobal.fighters.length<creepLimitsGlobal.fighters)roleSpawn.spawnCreep(spawn,LIB_MISC.ROLES.FIGHTER)
else{if(!(creepsLocal.claimers.length<=i&&creepsLocal.claimers.length<spawn.room.memory.claimerLimit&&creepsGlobal.claimers.length<creepLimitsGlobal.claimers))continue
roleSpawn.spawnCreep(spawn,LIB_MISC.ROLES.CLAIMER)}if(spawn.spawning)break}if(!spawn.spawning)return ERR_NOT_FOUND})()},spawnCreep:function(spawn,role,dryRun){dryRun=!!dryRun
let name="",partTypes=[WORK,CARRY,RANGED_ATTACK,HEAL,ATTACK,CLAIM],partRatios={movesPerPart:1,useTough:!1}
...
if(exitLoop)break
exitLoop=!1
for(let p=0;p<partTypes.length;p++)if(partRatios[partTypes[p]]>0&&partRatios[partTypes[p]]>=partCounts[partTypes[p]]/movelessParts&&energyTotal>=energyCost+BODYPART_COST[partTypes[p]]+neededMovesCost){partCounts.total++,partCounts[partTypes[p]]++,energyCost+=BODYPART_COST[partTypes[p]],exitLoop=!0
break}if(!exitLoop){if(!(partRatios.useTough&&energyTotal>=energyCost+BODYPART_COST[TOUGH]+neededMovesCost))break
partCounts.total++,partCounts[TOUGH]++,energyCost+=BODYPART_COST[TOUGH]}}partTypes=[TOUGH,WORK,CARRY,MOVE,CLAIM,RANGED_ATTACK,ATTACK,HEAL]
let bodyParts=[]
for(let p=0;p<partTypes.length;p++)for(;partCounts[partTypes[p]]>0;)partCounts[partTypes[p]]--,bodyParts.push(partTypes[p])
let directions=[BOTTOM,TOP,RIGHT,LEFT,BOTTOM_RIGHT,BOTTOM_LEFT,TOP_RIGHT,TOP_LEFT]
dryRun&&console.log(bodyParts.length,bodyParts)
for(let i=0;1;i++){let code=spawn.spawnCreep(bodyParts,name+i,{memory:{role:role},dryRun:dryRun,directions:directions})
